...
function generateSVG(languages, totalBytes) {
  const maxBarWidth = 270; // Ancho máximo de la barra en píxeles
  let yOffset = 55;

  const itemsSvg = languages.map(([lang, data], index) => {
    const percentage = totalBytes > 0 ? ((data.bytes / totalBytes) * 100).toFixed(1) : '0';
    // Calcular el ancho real en píxeles
    const barWidth = totalBytes > 0 ? Math.max(((data.bytes / totalBytes) * maxBarWidth), 4).toFixed(1) : '0';
    ...
    const item = `
      <g class="lang-row" transform="translate(25, ${yOffset})">
        ...
        <!-- Barra de progreso con width explícito + animación por escalado -->
        <rect class="bar bar-${index}" x="0" y="8" width="${barWidth}" height="7" rx="3.5" fill="${data.color}" />
      </g>
    `;
    yOffset += 45;
    return item;
  ...
  // Animación usando escalado horizontal compatible
  const keyframesStyle = languages.map((_, index) => `
    @keyframes scaleBar${index} {
      from { transform: scaleX(0); }
      to { transform: scaleX(1); }
    }
    .bar-${index} {
      transform-origin: left;
      animation: scaleBar${index} 0.8s cubic-bezier(0.25, 1, 0.5, 1) forwards;
    }
  `).join('\n');
